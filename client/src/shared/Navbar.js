...
const Navbar = ({edit, user, logout, toggleEdit, editPage, history}) => (
	<div>
		<div className='title'>
			Friends of Kowak
		</div>
		...
		<div id={7} className={'navbar'}>
			...
			</Link>
			{
				user ?
				<div>
					<Link to={''} className='login navItem' onClick={() => logout(history)}>
						Logout
					</Link>
						...
						<div>
							...
							<button className='formButton isConfirm navItem' onClick={editPage}>
								<span role='img' aria-label='Submit'>✔︎</span>
								 ...
								 :
						<div>
							{user.editor ? <button className='formButton isEdit navItem' onClick={toggleEdit}>✎</button> : null}
						</div>
					}
				</div>
						 :
